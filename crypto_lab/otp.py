...
LETTER_FREQUENCY = 'etaoinshrdlcumwfgypbvkjxqzﬁ'
LETTER_FREQUENCY += LETTER_FREQUENCY.upper()
LETTER_FREQUENCY += " :,-()"
...
def main():
  diff_table = {}
  characters = string.ascii_letters + string.digits + ' :,.-()ﬁ'
  for l1 in characters:
    for l2 in characters:
      diff = strxor(l1, l2)
      diff_table[diff] = diff_table.get(diff, '') + l1 + l2
  for diff in diff_table:
    diff_table[diff] = ''.join(sorted(set(diff_table[diff])))
    print diff.encode('hex'), diff_table[diff]
  ...
  for i in range(len(TARGET)):
    ...
    for c in CIPHER_TEXTS:
      diff = strxor(TARGET[i], c[i])
      ls = diff_table.get(diff, '')
      letters = filter(lambda x: x in ls, letters)
    print i, letters
    if len(letters) == 1:
      message += letters[0]
    elif not letters:
...
#
#
#   decrypted = 'The secret message is: When using a stream cipher, never use the key more than once'
#   key = ''.join([strxor(l, t) for l, t in zip(decrypted, TARGET)])
#   for c in CIPHER_TEXTS:
#     print strxor(key, c)
#
#   cracked_ciphers = """
...
# 9 The Concise OxfordDictionary (2006) deﬁnes crypto as the art of  writing o r sol""".split('\n')
#   print
#   for i in range(len(cracked_ciphers)):
#     key = strxor(cracked_ciphers[i], CIPHER_TEXTS[i])
