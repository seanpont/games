...
def find_keys_with_highest_value(a):
  """
  Takes a map and returns the keys with the highest values
  :param a: a map where the values are comparable
  :return a list of the keys with the highest values
  """
  return [x[0] for x in a.items() if x[1] == max(a.values())]
...
def find_items_that_occur_once(xs):
  ...
  """
  return [x[0] for x in map_count(xs).items() if x[1] == 1]
